...
export const deleteImage = async (req: IncomingMessage, res: Response) => {
    //Destruct the data sent from req.body 
    // const uploader = async (path) => await uploads(path, "Images")
    const { certId, id } = req.params
    try {
        ...
        if (req.method === 'DELETE') {
            ...
            const cerObject = certfication.filter((e)=>
                e._id.toString() === certId
            )
            if(!certfication){
                return res.status(404).json({
                    message: "profile not found",
            ...
            const filesarray = cerObject[0].files
            const newfilesarray = filesarray.filter((image) =>
                image.id !== "Images/" + id
            )
            verifyItem.certfication.filter((e) =>
                e._id.toString() === certId
