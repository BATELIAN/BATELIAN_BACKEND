...
export const addPortImage = async (req: IncomingMessage, res: Response) => {
    ...
    try {
        ...
        if (req.files) {
            ...
            const profileObje = portfolio.filter((e)=> e._id.toString() === id)
            const filesarray = profileObje[0].files
            filesarray.push(url);
            verifyItem.save();
